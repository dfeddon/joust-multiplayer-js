...
game_core.prototype.roundComplete = function(port, round) {
    ...
    if (round.stage === 1) // game round complete
    {
        ...
        for (var z = p.length - 1; z >= 0; z--) {
            ...
            if (p[z].team === 1) // red
                teamRed.push(p[z]);
            else if (p[z].team === 2) // blue
                teamBlue.push(p[z]);
            else(Math.random() > 0.5) ? teamRed.push(p[z]) : teamBlue.push(p[z]);
        }
        console.log("teamRed/Blue", teamRed.length, teamBlue.length);
        ...
        for (var x = 0; x < 4; x++) {
            ...
            losersRed.push(orderedRed.splice(rngRed - 1, 1));
            losersBlue.push(orderedBlue.splice(rngBlue - 1, 1));
        }
        console.log("top10:", orderedRed.length, orderedBlue.length);
        console.log("* losers", losersRed.length, losersBlue.length);
        // shuffle losers
        losersRed = _.shuffle(losersRed);
        losersBlue = _.shuffle(losersBlue);
